...
- **Owned email programme** (contacts live in our Postgres; Amazon SES is only the sending pipe). Phase 1 is done: the `contacts` + `email_suppressions` tables, the Contacts tab, and the public `POST /api/subscribe` (pending contacts only, no sending).
  ...
  - **The addresses printed on the website do not exist yet (found 2026-09-25).** A delivery test to each mailbox the site publishes — `hello@`, `advertise@`, `press@` — was accepted by SES and then hard-bounced by Microsoft 365 with `550 5.1.10 RESOLVER.ADR.RecipientNotFound`. The tenant answers for `transit411.net` but has no mailbox or alias by those names, so **every visitor who emails the contact page today gets a bounce.** Create the three mailboxes (or alias them onto one real inbox) in Microsoft 365 before launch. All three are now in `email_suppressions` as a side effect of the bounce — which is the feedback loop working correctly — so after the mailboxes exist, delete those three rows and retest, in that order.
