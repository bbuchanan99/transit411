...
- **Owned email programme** (contacts live in our Postgres; Amazon SES is only the sending pipe). Phase 1 is done: the `contacts` + `email_suppressions` tables, the Contacts tab, and the public `POST /api/subscribe` (pending contacts only, no sending).
  ...
  - **The addresses printed on the website (fixed 2026-09-25).** `hello@`, `advertise@` and `press@` were published on the contact page but did not exist in Microsoft 365 — a delivery test to each hard-bounced with `550 5.1.10 RESOLVER.ADR.RecipientNotFound`, so every visitor who used the contact page got a bounce. They had most likely been GoDaddy forwards that died when the root MX moved to Microsoft 365. Now created as **shared mailboxes** (free, no licence, native Send As, and `advertise@`/`press@` can be delegated later without migrating). Re-tested clean: SES accepted all three and no bounce followed within a minute, where the failures bounced in about two seconds.
