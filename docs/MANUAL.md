...
- **Acting on the set.** The suggested set is selectable: tick the ones you want, **Approve selected** or **Skip selected**, or use the per-row **Approve** / **Skip** — the same endpoints the queue cards use, so the AI still never acts. A row you have acted on dims, shows what happened and offers **Undo** (back to pending) rather than disappearing.
...
### 3.3b Pictures: three outcomes, chosen at publish time
- **A chosen picture** — picked in the Publish tab's image picker from what the article offers, or pasted. Stored as `image_url` + `image_source` (`candidate` / `manual`).
- **Nothing chosen** — the site falls back to that pillar's **house graphic** (`site/public/images/house/*.png`). Always safe: it is our own artwork.
- **No picture at all** — `image_source = 'none'`. The story runs as text: no photo and no house graphic, on the site *and* in The Wire. Set with **Publish with no picture**, or **No picture at all** in the picker. The one exception is the `og:image` / `twitter:image` share tag, which still uses our pillar graphic so a shared link does not render a blank card — metadata only, never on the page.

### 3.3c Image & logo library (branch `feature/image-library`)
- **`image_library.py`** — one table, `image_library`, holding agency logos and topic stock photos. **Provenance is a CHECK constraint, not a convention**: source, source_url, licence and attribution must all be non-empty or the row cannot be inserted, whatever a future caller or hand-written INSERT tries. `check_provenance()` refuses it earlier with a message naming what is missing.
- **Everything lands as `candidate`.** A logo is a trademark and a stock photo is a licence agreement; approving is a human judgement made on the LAN. Re-running a fetch never touches a decision already made.
- **`logos.py`** — Wikimedia only; we never scrape an agency's site. **Most US transit logos are NOT freely licensed** — they sit on English Wikipedia under a fair-use rationale that covers Wikipedia and not us. Each asset is classed **free / non-free / unclear** and the class is shown as a coloured warning beside the Approve button.
- **`stock.py`** — Unsplash + Pexels, both requiring a credit, which is stored and displayed. A missing `UNSPLASH_KEY` / `PEXELS_KEY` skips that provider loudly and blocks nothing.
- **`image_recommend.py`** — the cascade: chosen picture → the agency's approved logo → an approved topic photo → the site's clean no-image state. Only **approved** assets are ever offered. Most stories need no model call; the model is asked only to choose between several plausible stock images.
- **`/api/posts` fills the last two steps as the site reads a post**, and returns `image_attribution` / `image_license` / `image_source_url` with it. The article page prints that credit — an image shown without the credit its licence requires is a breach, not a detail.
- **Review:** Web Content → **Image library**, with a coverage strip. None of `/api/images/library*` is on the read-only allowlist.
- **Matching an agency to a Wikipedia article is the hard part**, and these traps are all fixed — don't reintroduce them: a bare acronym matches itself perfectly (searching "CTA" found the article *CTA*, "BART" found *Bart Simpson*), so the **most specific name is searched first**; Wikipedia's parenthetical disambiguator must be stripped before comparing (*PATH (rail system)* scored 0.50 with it attached); among equal matches prefer a **transit-sounding title**; **disambiguation pages** match perfectly and carry no logo, so they're detected via `pageprops` and skipped; and **one shared word is not a match** — the alias "City of Madison" reduces to `{madison}` and scored 1.00 against *Madison County Transit*, a different agency in a different state.
- **Picking the logo file** is the other half: `pageimages` returns the article's *lead* image, which on a transit article is a photo of a bus, so the infobox's own `logo=` field is read from the wikitext. A plain `image=` counts only when it's a **vector** (MBTA's infobox says `image = MBTA.svg`). Values can carry templates — Caltrain's reads `Caltrain_logo.svg{{!}}class=skin-invert`. When no plausible logo is found it returns a **miss rather than a photograph**; a bus picture stored as a logo would be worse than the house graphic it replaces and would make the coverage numbers a lie.
- **Wikimedia rate-limits anonymous readers.** Calls are paced at 1/sec with `Retry-After` honoured, and a 429 **stops the run** rather than recording the remaining agencies as misses.
- **Picking a picture from the library.** The Publish tab's image picker now has two library sections: **Suggested for this story** — the top few approved assets, agency logo first, then topic-matched photos with the model putting the best one first and saying why — and **Everything else in the library** to browse. Suggestions are a shortlist to look at; you pick. A library pick stores `content_posts.image_library_id`, so the post keeps the **link** rather than a copy of the credit: a correction to a licence or credit reaches every post using it instead of going stale in a dozen places. `GET /api/images/suggest?item_id=|post_id=` is the endpoint (private).
...
- **Owned email programme** (contacts live in our Postgres; Amazon SES is only the sending pipe). Phase 1 is done: the `contacts` + `email_suppressions` tables, the Contacts tab, and the public `POST /api/subscribe` (pending contacts only, no sending).
  ...
  - **The addresses printed on the website (fixed 2026-09-25).** `hello@`, `advertise@` and `press@` were published on the contact page but did not exist in Microsoft 365 — a delivery test to each hard-bounced with `550 5.1.10 RESOLVER.ADR.RecipientNotFound`, so every visitor who used the contact page got a bounce. They had most likely been GoDaddy forwards that died when the root MX moved to Microsoft 365. Now **aliases (proxy addresses) on `admin@transit411.net`**, so all three land in one inbox. Re-tested clean: SES accepted all three and no bounce followed within a minute, where the failures bounced in about two seconds. **Replying as them needs a second, separate switch:** Exchange admin center → Settings → Mail flow → *Turn on sending from aliases* (`Set-OrganizationConfig -SendFromAliasEnabled $true`). Without it an alias receives mail but every reply goes out as `admin@` — which reads as if the alias never worked. It is on, and all three show in the From list in Outlook on the web; OWA is the place to test it, because it reads the setting live while desktop Outlook holds a cached profile until it is fully restarted. If `advertise@` or `press@` ever goes to someone else, drop the alias and make it a real shared mailbox then.
  - **The SES suppression list is not our suppression list — this cost an hour.** After those hard bounces, clearing our `email_suppressions` rows and resending looked like it worked (`accepted by SES: 3 of 3`) but every message came straight back as `Permanent/OnAccountSuppressionList`. **SES keeps its own account-level suppression list inside AWS**, adds hard-bounced addresses to it automatically, and then silently discards mail to them and synthesises the bounce itself — the recipient's server is never contacted, so the test proves nothing. Clear it in the SES console (region us-east-1 → Suppression list) or with `aws sesv2 delete-suppressed-destination`. The send-only IAM user **cannot** clear it (`AccessDeniedException` on both list and delete) and deliberately stays that way: a leaked key must not be able to remove bounce protections. Treat suppression-list edits as a console job.
...
- **Region:** us-east-1. **Root domain mail is on Microsoft 365** (MX `transit411-net.mail.protection.outlook.com`, SPF `include:spf.protection.outlook.com`) — it moved off GoDaddy at some point; the sending subdomain `mail.transit411.net` is separate and unaffected. Never point the root MX at SES.
